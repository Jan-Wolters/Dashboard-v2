...
async function getInfo(req, res) {
  try {
    ...
    const combinedRows = await databaseManager.query(combinedQuery, [companyId, companyId]);
    ...
    if (Array.isArray(combinedRows)) {
      ...
      companiesMap.forEach((company) => {
        company.sessions.sort((a, b) => new Date(b.session_endTime) - new Date(a.session_endTime));
      });
