...
class SNMPProcessor {
	...
	mysqlConfig: { host: string; user: string; password: string; database: string; };
	constructor() {
	...
	async initialize() {
		try {
			const companies: any = await this.getCompaniesFromDatabase();
	...
	async performServerSnmpForCompany(company: any) {
	...
	async performFirewallSnmpForCompany(company: any) {
		const { company_id, ip, port, community } = company;
			...
			try {
				...
				for (const oidName in oids) {
					if (oids.hasOwnProperty(oidName)) {
						const oid = oids[oidName as keyof typeof oids];
	...
	async insertSNMPDataIntoDatabase(company_id: any, snmpData: any) {
		const connection = await mysql.createConnection(this.mysqlConfig);
		...
		try {
			...
			if (snmpData.server && snmpData.server.sizeOID) {
				const inserts = snmpData.server.sizeOID.map((size: any, index: any) => {
	...
	async createSnmpDataWalkTableIfNotExists(connection: any) {
		try {
	...
	async createSnmpDataGetTableIfNotExists(connection: any) {
		try {
			await connection.execute(`
	...
	async createFirewallSnmpDataTableIfNotExists(connection: any) {
		try {
			await connection.execute(`
	...
	async insertDataIntoTable(connection: any, tableName: any, ...values: any[]) {
		try {
	...
	async performSnmpWalk(session: any, oid: any) {
		return new Promise((resolve, reject) => {
			session.getSubtree({ oid }, (error: any, varbinds: any) => {
				if (error) {
				...
				const values = varbinds
					.filter((varbind: any, index: any) => index === 0 || index === 1)
					.map((varbind: any) => varbind.value);
				resolve(values);
			});
		});
	}

	async performSnmpGet(session: any, oid: any) {
		return new Promise((resolve, reject) => {
			session.get({ oid: oid }, (error: any, varbinds: any) => {
				if (error) {
